...
export async function POST(req: Request) {
  try {
    ...
    if (error || !interview) {
      console.log("[v0] Interview not found:", error)
      return Response.json({ error: "Interview not found" }, { status: 404 })
    }

    console.log("[v0] Interview found:", interview.job_position, interview.difficulty_level)
    ...
    if (existingQuestions && existingQuestions.length > 0) {
      console.log("[v0] Questions already exist, returning existing questions")
    ...
    if (insertError) {
      console.error("[v0] Error inserting questions:", insertError)
